...
if [ "$AIRFLOW__CORE__EXECUTOR" = "CeleryExecutor" ]; then
  ...
  if [ -z "$AIRFLOW__CELERY__BROKER_URL" ]; then
    ...
    # Derive useful variables from the AIRFLOW__ variables provided explicitly by the user
    # When the user passes a full broker URL, we still want to know the host and
    # port for the wait_for_port helper. Extract them from BROKER_URL instead of
    # relying on the Postgres connection string.
